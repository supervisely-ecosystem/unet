...
def update_and_send_progress(count, index, api: sly.Api, task_id, progress: sly.Progress):
    progress.iters_done(count)
    _send_progress_ui(index, api, task_id, progress)
...
def set_and_send_progress(count, index, api: sly.Api, task_id, progress: sly.Progress):
    progress.set_current_value(count)
    _send_progress_ui(index, api, task_id, progress)


def _send_progress_ui(index, api: sly.Api, task_id, progress: sly.Progress, stdout_print=False):
    fields = [
...
def _update_progress_ui(index, api: sly.Api, task_id, progress: sly.Progress, stdout_print=False):
    if progress.need_report():
        _send_progress_ui(index, api, task_id, progress, stdout_print)


def get_progress_cb(index, message, total, is_size=False, min_report_percent=5, upd_func=update_progress, set_zero=True):
